...
export default function Home({ params: paramsPromise }) {
  ...
  const { user, loading: authLoading, logout, checkAuth } = useUser();
  ...
  // Fetch health status (API Doctor)
  useEffect(() => {
    if (user) {
        fetch('/api/admin/doctor')
            .then(res => res.json())
            .then(data => setHealthStatus(data.diagnostics))
  ...
  const fetchFiles = useCallback(async (isLoadMore = false) => {
    try {
      ...
      if (response.status === 401) {
          // If we have a user in context but get 401, session might have expired - Show login
          // If no user, it's just a guest view - Stay quiet
          if (user) {
            setShowLogin(true);
          }
          setLoading(false);
  ...
  // Handle deep-link login request
  useEffect(() => {
  ...
  useEffect(() => {
    if (user) {
      ...
      setLoading(false); // Stop loading skeleton for guests
    }
  }, [refreshTrigger, currentPath, debouncedSearch, user]);
  ...
  return (
    <DropZone onFilesDropped={(files) => setDroppedFiles(files)}>
                  ...
                  <div className="flex items-center gap-3">
                    <div className="hidden lg:flex flex-col items-end">
                      ...
                      <div className="flex items-center gap-2">
                           {healthStatus && config.isEnterprise && (
                              <div
                                  title={healthStatus.can_process_video ? "Streaming Engine: Online" : "Streaming Engine: Offline (Check Redis/Worker)"}
                                  className={`flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider ${healthStatus.can_process_video ? 'bg-blue-500/10 text-blue-500' : 'bg-amber-500/10 text-amber-500'}`}
                              >
                                  <span className={`w-1 h-1 rounded-full ${healthStatus.can_process_video ? 'bg-blue-500' : 'bg-amber-500 animate-pulse'}`}></span>
                                  {healthStatus.can_process_video ? 'Streaming Active' : 'Engines Offline'}
                              </div>
                           )}
      ...
      <div className="flex flex-col min-h-[calc(100vh-80px)]">
        <div className="flex flex-col xl:flex-row gap-6 w-full px-4 xl:px-8 py-6 flex-1">
