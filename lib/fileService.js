...
export class FileService {
  ...
  async handleUploadChunk(userId, body) {
    ...
    if (partNum === 1) {
      ...
      if (!fileRecord) {
        console.log(`[FileService] Creating new file record for ${file_id}. Fragmented: ${body.is_fragmented}`);
        fileRecord = await fileRepository.save({
          ...
          is_fragmented: body.is_fragmented === true || body.is_fragmented === 'true'  // Ensure boolean
        });
