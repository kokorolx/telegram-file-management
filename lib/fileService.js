...
const CHUNK_SIZE = 50 * 1024 * 1024; // 50MB - Telegram Bot API Limit
 ...
 */
/**
 * Uploads a file with encryption and splitting using a provided key.
 * @param {File} file
 * @param {Buffer} key
 * @param {string} folderId
 * @param {string} [description]
 * @param {string} [tags]
 */
export async function processEncryptedUploadWithKey(file, key, folderId, description, tags) {
    console.log(`Starting encrypted upload (server key) for ${file.name}, size: ${file.size}`);

    // 2. Prepare metadata
    const fileId = uuidv4();
    const fileExt = getFileExtension(file.name);

    // 3. Save Main File Record (First, to satisfy FK)
    await insertFile({
        id: fileId,
        folder_id: folderId,
    ...
    // 4. Process Stream
    await processFileStream(file, key, fileId);
...
/**
 * Legacy wrapper: derives key from password then uploads.
 */
export async function processEncryptedUpload(file, masterPassword, folderId, description, tags) {
    ...
    return processEncryptedUploadWithKey(file, key, folderId, description, tags);
}
