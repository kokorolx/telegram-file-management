...
// Video file extensions for random chunk size feature
...
export async function processEncryptedUploadWithKey(fileBuffer, key, folderId, filename, description, tags, fileId = null) {
    ...
    // Process Buffer (videos get random chunk sizes)
    await processBufferWithEncryption(fileBuffer, key, id, isVideo);
 ...
 * @param {boolean} isVideo - If true, use random chunk sizes (1-5MB) for obfuscation
 */
async function processBufferWithEncryption(fileBuffer, key, fileId, isVideo = false) {
     let partNumber = 1;
     const startTime = Date.now();
     const chunkSizes = [];
     ...
     while (offset < fileBuffer.length) {
         // Use random size for videos, fixed size for other files
         const chunkSize = isVideo
             ? getRandomChunkSize()
             : DEFAULT_CHUNK_SIZE;
     ...
     if (isVideo) {
         console.log(`[FILE-SERVICE] ${fileId} - 🎲 Video mode: using random chunk sizes (${MIN_VIDEO_CHUNK_SIZE/1024/1024}MB - ${MAX_VIDEO_CHUNK_SIZE/1024/1024}MB)`);
