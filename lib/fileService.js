...
const DEFAULT_CHUNK_SIZE = 2.5 * 1024 * 1024; // 2.5MB average
const MIN_VIDEO_CHUNK_SIZE = 2 * 1024 * 1024; // 2MB minimum
const MAX_VIDEO_CHUNK_SIZE = 3 * 1024 * 1024; // 3MB maximum
 ...
 */
export async function processEncryptedUploadWithKey(fileBuffer, key, userId, folderId, filename, description, tags, fileId = null) {
    const id = fileId || uuidv4();
    console.log(`Starting encrypted upload (server key) for ${filename}, size: ${fileBuffer.length}, fileId: ${id}, userId: ${userId}`);
    ...
    await processBufferWithEncryption(fileBuffer, key, id, userId);
 ...
 */
export async function processEncryptedUpload(fileBuffer, masterPassword, userId, folderId, description, tags, fileId, filename) {
    ...
    // Pass userId through to processEncryptedUploadWithKey for user_id isolation
    return processEncryptedUploadWithKey(optimizedBuffer, key, userId, folderId, filename, description, tags, fileId);
}
 ...
 */
async function processBufferWithEncryption(fileBuffer, key, fileId, userId) {
     let partNumber = 1;
     const startTime = Date.now();
     const chunkSizes = [];
     ...
     try {
         ...
         for (let i = 0; i < totalChunks; i++) {
             ...
             await processChunk(chunk, key, fileId, partNumber++, userId);
         }
...
async function processChunk(chunkBuffer, key, fileId, partNumber, userId) {
     ...
     const tgFileId = await sendFileToTelegram(userId, encrypted, partFilename);
