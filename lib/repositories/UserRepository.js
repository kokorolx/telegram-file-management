...
export class UserRepository extends BaseRepository {
  ...
  async hasRecoveryCodes(userId) {
    const user = await this.findById(userId);
    return user && user.recovery_codes_enabled === true;
  }

  async setRecoveryCodesGenerated(userId, flag) {
    const repo = await this.getRepository();
    return repo.update(userId, {
      recovery_codes_generated_on_first_setup: flag,
      updated_at: new Date()
    });
  }
}
