...
/**
 * Check if email collection feature is enabled
 *
 * Environment variable: NEXT_PUBLIC_ENABLE_EMAIL_COLLECTION
 * Default: 'false'
 *
 * @returns {boolean} true if email collection is enabled
 */
export const isEmailCollectionEnabled = () => {
    return process.env.NEXT_PUBLIC_ENABLE_EMAIL_COLLECTION === 'true';
};

/**
 * Check if MP4 fragmentation feature is enabled
 *
 * Environment variable: NEXT_PUBLIC_ENABLE_MP4_FRAGMENTATION
 * Default: 'false'
 *
 * @returns {boolean} true if MP4 fragmentation is enabled
 */
/**
 * Check if MP4 fragmentation is enabled
 * 
 * DISABLED by default due to FFmpeg WASM codec-copy instability
 * causing decryption failures with fragmented chunks.
 * 
 * Environment variable: NEXT_PUBLIC_ENABLE_MP4_FRAGMENTATION
 * Default: 'false' (disabled)
 * 
 * @returns {boolean} true if fragmentation should be used
 */
export const isMp4FragmentationEnabled = () => {
    return process.env.NEXT_PUBLIC_ENABLE_MP4_FRAGMENTATION === 'true';
};

/**
 * Check if FFmpeg multi-threading is enabled
 *
 * Environment variable: NEXT_PUBLIC_FFMPEG_MULTITHREAD
 * Default: 'false'
 *
 * @returns {boolean} true if multi-threading should be used
 */
export const isFfmpegMultiThreadEnabled = () => {
    return process.env.NEXT_PUBLIC_FFMPEG_MULTITHREAD === 'true';
};
