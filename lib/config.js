...
export const config = {
  ...
  // Video Streaming & Chunk Configuration
  maxConcurrentChunkFetches: parseInt(process.env.MAX_CONCURRENT_CHUNK_FETCHES || '3', 10),
  videoPlaybackBufferThreshold: parseInt(process.env.VIDEO_PLAYBACK_BUFFER_THRESHOLD || '3', 10),
  
  // Cache Limits (in bytes)
  maxImageCacheSize: parseInt(process.env.MAX_IMAGE_CACHE_SIZE || String(100 * 1024 * 1024), 10), // 100MB default
  maxVideoCacheSize: parseInt(process.env.MAX_VIDEO_CACHE_SIZE || String(500 * 1024 * 1024), 10), // 500MB default
  maxAudioCacheSize: parseInt(process.env.MAX_AUDIO_CACHE_SIZE || String(500 * 1024 * 1024), 10), // 500MB default
};
