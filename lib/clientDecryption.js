 ...
 */
async function getDEK(file, masterKey) {
...
export async function fetchAndDecryptChunk(fileOrId, partMetadata, key) {
  ...
  try {
    ...
    const response = await fetch(`/api/chunk/${encodeURIComponent(fileId)}/${part_number}`);
  ...
  try {
    ...
    async function worker() {
      while (queue.length > 0) {
        ...
        try {
          results[index] = await fetchAndDecryptChunk(file.id, part, dek);
        } catch (err) {
 ...
 */
export async function verifyFileKey(file, masterKey, partMetadata) {
  if (!file || !masterKey || !partMetadata || partMetadata.length === 0) {
    return false;
  }
 ...
 */
export async function createDecryptedStream(file, masterKey, partMetadata) {
  const dek = await getDEK(file, masterKey);
  ...
  return new ReadableStream({
    async pull(controller) {
      try {
        ...
        const decrypted = await fetchAndDecryptChunk(file.id, part, dek);
