 ...
 */
export async function fetchAndDecryptChunk(fileId, partMetadata, key) {
  try {
    const {
      part_number,
      iv,               // Hex-encoded IV
  ...
  try {
    ...
    async function worker() {
      while (queue.length > 0) {
        ...
        try {
          results[index] = await fetchAndDecryptChunk(fileId, part, key);
        } catch (err) {
 ...
 */
export async function verifyFileKey(fileId, key, partMetadata) {
  if (!fileId || !key || !partMetadata || partMetadata.length === 0) {
    return false;
  }
  ...
  try {
    ...
    await fetchAndDecryptChunk(fileId, firstPart, key);
    return true;
  } catch (err) {
    console.warn(`Key verification failed for file ${fileId}:`, err.message);
    return false;
  }
}
