...
export async function fetchAndDecryptFullFile(fileId, key, partMetadata) {
  ...
  try {
    // Parallel fetching and decryption
    const decryptionPromises = partMetadata.map(part =>
      fetchAndDecryptChunk(fileId, part, key)
    );

    const decryptedChunks = await Promise.all(decryptionPromises);

    // Combine all chunks into single blob
    const blob = new Blob(decryptedChunks.map(c => c.buffer));
