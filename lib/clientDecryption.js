...
export async function fetchAndDecryptChunk(fileOrId, partMetadata, key, shareToken = null, s3ConfigReencrypted = null) {
  ...
  try {
    ...
    if (is_compressed) {
      console.log(`[DOWNLOAD] Decompressing chunk ${part_number}...`);
      const decompressed = await decompressData(decrypted);
      console.log(`✓ Decrypted & Decompressed chunk ${part_number} (${decrypted.length} -> ${decompressed.length} bytes)`);
      return decompressed;
    }
...
async function prepareS3ConfigForDownload(masterPassword) {
  ...
  cachedS3ConfigPromise = (async () => {
    try {
      ...
      console.log(`✓ S3 config prepared for download (personal bucket)`);
 ...
 */
export async function fetchAndDecryptFullFile(file, masterKey, partMetadata, shareToken = null, isDEK = false, masterPassword = null) {
  ...
  try {
    ...
    async function worker() {
      while (queue.length > 0) {
        ...
        try {
          results[index] = await fetchAndDecryptChunk(file.id, part, dek, shareToken, s3ConfigReencrypted);
        } catch (err) {
