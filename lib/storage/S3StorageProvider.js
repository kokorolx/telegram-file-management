...
export class S3StorageProvider extends StorageProvider {
  ...
  async uploadChunk(userId, buffer, filename, s3Config = null, fileId = null) {
    ...
    try {
      ...
      console.log(`[S3] Uploaded ${key} to bucket ${s3Config.bucket} (Class: ${s3Config.storageClass || 'STANDARD'})`);
      return key;
    } catch (err) {
  ...
  async getDownloadUrl(userId, storageId, s3Config = null) {
    ...
    try {
      ...
      console.log(`[S3] Generated presigned URL for ${storageId}`);
      return url;
    } catch (err) {
  ...
  async deleteChunk(storageId, s3Config = null) {
    ...
    try {
      ...
      await client.send(command);
      console.log(`[S3] Deleted ${storageId} from bucket ${s3Config.bucket}`);
      return true;
    } catch (err) {
