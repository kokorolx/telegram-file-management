  ...
  try {
    ...
    await pool.query(`
      CREATE TABLE IF NOT EXISTS files (
        id TEXT PRIMARY KEY,
        folder_id TEXT,
        telegram_file_id TEXT, -- Nullable now because it might be split
...
export async function insertFile(fileData) {
  ...
  try {
    await pool.query(
      `INSERT INTO files (id, folder_id, telegram_file_id, original_filename, file_size, file_type, mime_type, description, tags)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
      [id, folder_id || null, telegram_file_id, original_filename, file_size, file_type, mime_type, description || null, tags || null]
    );
    return fileData;
  } catch (err) {
