...
export async function initDb() {
  try {
    ...
    // Create index for shared links token
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_shared_links_token ON shared_links(token)`);
    ...
    try {
        ...
        // Resumable upload support (Phase 1)
        await pool.query(`ALTER TABLE files ADD COLUMN IF NOT EXISTS is_complete BOOLEAN DEFAULT false`);
        await pool.query(`ALTER TABLE files ADD COLUMN IF NOT EXISTS total_parts_expected INTEGER`);
        await pool.query(`ALTER TABLE files ADD COLUMN IF NOT EXISTS chunk_sizes JSONB`);

        // Indexes for resume detection
        await pool.query(`
        ...
        await pool.query(`CREATE INDEX IF NOT EXISTS idx_files_chunk_sizes ON files(user_id) WHERE chunk_sizes IS NOT NULL`);

        // Upload sessions table for tracking
        await pool.query(`
        ...
        await pool.query(`CREATE INDEX IF NOT EXISTS idx_upload_sessions_user ON upload_sessions(user_id)`);
        await pool.query(`CREATE INDEX IF NOT EXISTS idx_upload_sessions_expires ON upload_sessions(expires_at)`);
        ...
        await pool.query(`ALTER TABLE file_parts ADD COLUMN IF NOT EXISTS is_compressed BOOLEAN DEFAULT false`);
        ...
        await pool.query(`ALTER TABLE files ADD COLUMN IF NOT EXISTS encrypted_file_key TEXT`);
        await pool.query(`ALTER TABLE files ADD COLUMN IF NOT EXISTS key_iv TEXT`);
        await pool.query(`ALTER TABLE files ADD COLUMN IF NOT EXISTS encryption_version INTEGER DEFAULT 1`);
        ...
        // S3/R2 Backup Storage migration
        await pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS organization_id TEXT`);
        await pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS encrypted_s3_config TEXT`);
        await pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS s3_config_iv TEXT`);
        await pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS s3_config_tag TEXT`);
        await pool.query(`ALTER TABLE file_parts ADD COLUMN IF NOT EXISTS backup_storage_id TEXT`);
        await pool.query(`ALTER TABLE file_parts ADD COLUMN IF NOT EXISTS backup_backend TEXT`);

        // Organizations table for group-level S3 configs
        await pool.query(`
        ...
        `);
        await pool.query(`CREATE INDEX IF NOT EXISTS idx_users_organization_id ON users(organization_id)`);

        // Recovery codes migration
        await pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS recovery_codes_enabled BOOLEAN DEFAULT FALSE`);
        await pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS recovery_codes_generated_on_first_setup BOOLEAN DEFAULT FALSE`);
        await pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS last_master_password_change TIMESTAMP`);
        ...
        `);
        await pool.query(`CREATE INDEX IF NOT EXISTS idx_recovery_codes_user_id ON recovery_codes(user_id)`);
        await pool.query(`CREATE INDEX IF NOT EXISTS idx_recovery_codes_user_used ON recovery_codes(user_id, used)`);
        await pool.query(`CREATE INDEX IF NOT EXISTS idx_recovery_codes_expires_at ON recovery_codes(expires_at)`);
        await pool.query(`CREATE INDEX IF NOT EXISTS idx_recovery_codes_expired ON recovery_codes(user_id, used, expires_at) WHERE used = FALSE`);
        } catch (e) {
        }
...
export async function insertFilePart(partData) {
  const { id, file_id, telegram_file_id, part_number, size, iv, auth_tag, is_compressed } = partData;
  try {
    await pool.query(
      `INSERT INTO file_parts (id, file_id, telegram_file_id, part_number, size, iv, auth_tag, is_compressed)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
      [id, file_id, telegram_file_id, part_number, size, iv, auth_tag, is_compressed || false]
    );
    return partData;
  } catch (err) {
