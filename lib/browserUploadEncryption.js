...
export async function encryptFileChunks(file, password, onProgress, folderId) {
  ...
  try {
    ...
    onProgress?.(0, Math.ceil(file.size / CHUNK_SIZE), 'Deriving encryption key...');
      ...
      try {
        ...
        while (buffer.length >= CHUNK_SIZE || (done && buffer.length > 0)) {
          const chunkData = buffer.slice(
            0,
            done && buffer.length < CHUNK_SIZE ? buffer.length : CHUNK_SIZE
          );
          ...
          // Encrypt chunk
          onProgress?.(partNumber, totalChunks, `Encrypting chunk ${partNumber}/${totalChunks}...`);
          const { encrypted_data, iv, auth_tag } = await encryptChunk(chunkData, key);

          // Upload encrypted chunk
          onProgress?.(partNumber, totalChunks, `Uploading chunk ${partNumber}/${totalChunks}...`);
    ...
    onProgress?.(totalChunks, totalChunks, 'Upload complete!');
