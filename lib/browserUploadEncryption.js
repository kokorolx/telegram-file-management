...
const MAX_CHUNK_SIZE = 3 * 1024 * 1024; // 3MB maximum (Vercel limit is ~4.5MB)
...
export async function encryptFileChunks(file, password, onProgress, folderId, abortSignal = null) {
  ...
  try {
    ...
    onProgress?.(0, Math.ceil(file.size / MIN_CHUNK_SIZE), 'Deriving encryption key...');
    const key = await deriveUploadKey(password);
    console.log(`✓ Encryption key derived`);

    // Step 3: Stream file and encrypt chunks
    ...
    let estimatedChunks = Math.ceil(file.size / ((MIN_CHUNK_SIZE + MAX_CHUNK_SIZE) / 2));

    console.log(`📦 Estimated chunks: ${estimatedChunks} (using random sizes ${MIN_CHUNK_SIZE / 1024 / 1024}MB - ${MAX_CHUNK_SIZE / 1024 / 1024}MB)`);
    ...
    while (true) {
       ...
       try {
         ...
         // Get random chunk size for this chunk
         const currentChunkSize = getRandomChunkSize();

         // Process full chunks
         while (buffer.length >= currentChunkSize || (done && buffer.length > 0)) {
           // Check again before processing each chunk
           if (abortSignal?.aborted) {
             throw new Error('Upload cancelled by user');
           }
           ...
           const chunkData = buffer.slice(
             0,
             done && buffer.length < currentChunkSize ? buffer.length : currentChunkSize
           );
           ...
           // Encrypt chunk
           onProgress?.(partNumber, estimatedChunks, `Encrypting chunk ${partNumber}/${estimatedChunks}...`);
           const { encrypted_data, iv, auth_tag } = await encryptChunk(chunkData, key);

           // Upload encrypted chunk
           onProgress?.(partNumber, estimatedChunks, `Uploading chunk ${partNumber}/${estimatedChunks}...`);
          ...
          onProgress?.(partNumber, partNumber, 'Upload complete!');
          console.log(`✅ All ${partNumber} chunks uploaded successfully`);
