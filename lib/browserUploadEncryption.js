...
} from './browserS3ConfigEncryption.js';
 ...
 * @param {boolean} isFragmented - Whether file was fragmented
 * @param {number} videoDuration - Duration in seconds for videos (optional)
 * @returns {Promise<{file_id, total_parts, filename}>}
 */
export async function encryptFileChunks(file, password, onProgress, folderId, abortSignal = null, targetFileId = null, resumeChunkStart = 1, savedChunkSizes = null, isFragmented = false, videoDuration = null) {
  if (!file) {
  ...
  try {
    ...
    // Step 1: Prepare S3 config if available
    let s3ConfigReencrypted = null;
    try {
      onProgress?.(0, 0, 'Checking S3 backup configuration...');

      // Fetch encryption salt (needed for decryption)
      const settingsRes = await fetch('/api/settings', { signal: abortSignal });
      const settings = await settingsRes.json();
      const salt = settings.encryptionSalt;

      // Fetch user's S3 config (encrypted with master password)
      const s3ConfigRes = await fetch('/api/upload/s3-config', { signal: abortSignal });
      if (s3ConfigRes.ok) {
        const s3Data = await s3ConfigRes.json();
        if (s3Data.hasConfig && s3Data.config) {
          // Decrypt S3 config with master password using the correct salt
          const decrypted = await decryptS3ConfigWithMasterPassword(
            s3Data.config,
            password,
            salt // Use the same salt that was used to encrypt it
          );
          ...
          s3ConfigReencrypted = await encryptS3ConfigWithServerKey(decrypted, publicKey);
        }
      }
    } catch (s3Err) {
      // Non-blocking: S3 config failure doesn't stop file upload
      console.warn(`[S3] Could not prepare S3 config: ${s3Err.message}`);
      s3ConfigReencrypted = null;
    }

    // Step 2: Use provided file ID or generate new one
    const fileId = targetFileId || generateFileId();

    // Step 3: Pre-calculate chunks for accurate progress reporting
    let chunkSizes;
    let totalParts;
    ...
    if (savedChunkSizes && savedChunkSizes.length > 0) {
      ...
      while (remainingSize > 0) {
        ...
        // For fragmented videos: first chunk must be large enough to include full init segment (ftyp+moov)
        // Typical fMP4 init is < 500KB (ftyp < 1KB + moov < 500KB), use 2MB for safety
        if (isFirstChunk && isFragmented) {
          size = Math.min(2 * 1024 * 1024, remainingSize); // First chunk: 2MB for fMP4 init segment
          isFirstChunk = false;
        } else {
    ...
    const processUpload = async (chunkData, pNum) => {
      ...
      // Upload encrypted chunk
       ...
       await uploadEncryptedChunk(
        ...
        pNum === 1 ? s3ConfigReencrypted : null,  // Send S3 config only on first chunk
        pNum === 1 ? isFragmented : false,  // Send is_fragmented flag only on first chunk
        pNum === 1 ? videoDuration : null  // Send video duration only on first chunk
       );
...
export async function uploadEncryptedChunk(
   ...
   encryptionVersion = 1,
   isCompressed = false,
   chunkPlan = null,  // Array of chunk sizes (sent only on first chunk)
   s3ConfigReencrypted = null,  // S3 config re-encrypted with server key (sent only on first chunk)
   isFragmented = false,  // Whether the file was fragmented for progressive playback
   videoDuration = null  // Duration in seconds (sent only on first chunk for videos)
 ) {
  try {
    ...
    // Include re-encrypted S3 config on first chunk only
    if (s3ConfigReencrypted) {
      body.s3_config_reencrypted = s3ConfigReencrypted;
    }
    ...
    // Include is_fragmented flag on first chunk only
    if (isFragmented) {
      body.is_fragmented = isFragmented;
      console.log(`[Upload] Sending is_fragmented=true for file ${fileId}`);
    }

    // Include video_duration on first chunk only (for videos)
    if (videoDuration) {
      body.video_duration = videoDuration;
      console.log(`[Upload] Sending video_duration=${videoDuration}s for file ${fileId}`);
    }
