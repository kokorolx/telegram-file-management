...
export async function wrapKey(dek, keyMaterial, salt) {
    let kek;
    const isBuffer = keyMaterial instanceof Uint8Array || (keyMaterial && keyMaterial.buffer instanceof ArrayBuffer);
...
export async function unwrapKey(wrappedKeyBase64, keyMaterial, salt, ivHex) {
    ...
    if (!ivHex || !wrappedKeyBase64) {
        throw new Error(`Missing decryption metadata: iv=${!!ivHex} key=${!!wrappedKeyBase64}`);
    }
    ...
    try {
        ...
        console.error('unwrapKey decryption failed:', {
            error: err.name,
            ivLen: iv.length,
            wrappedKeyLen: wrappedKey.length,
            isBufferKey: isBuffer,
            hasSalt: !!salt
        });
        throw err;
    }
